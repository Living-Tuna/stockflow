            ...
            return (
              <SidebarMenuItem key={link.href}>
                ...
                {hasChildren && sidebarState === 'expanded' && isExpanded && (
                  <SidebarMenuSub className="ml-11 border-l-2 border-primary/40 pl-3">
                      ...
                      return (
                        <SidebarMenuSubItem key={child.href}>
                          <SidebarMenuSubButton
                            ...
                            className={cn(
                              "py-2 rounded-lg data-[active=true]:bg-primary data-[active=true]:text-primary-foreground data-[active=true]:font-bold"
                            )}
                          >
...
function LocalSidebarNavInner() {
  ...
  return (
    <Sidebar className="border-r border-border" collapsible="icon">
      <SidebarHeader className="h-16 bg-gradient-to-r from-primary/20 via-primary/10 to-transparent border-b border-border">
        <div className={cn("flex items-center h-full", sidebarState === 'expanded' ? "justify-between pl-3 pr-2" : "justify-center")}>
          {sidebarState === 'expanded' ? (
            <Link href="/local" className="flex items-center gap-3 overflow-hidden">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground ring-1 ring-primary/40">
                <Store className="h-6 w-6" />
              </span>
              <span className="flex min-w-0 flex-col">
                <span className="truncate text-base font-semibold text-secondary-foreground">Local Mode</span>
                <span className="truncate text-xs text-secondary-foreground/60">Offline workspace</span>
              </span>
            </Link>
          ) : (
             <Tooltip>

            ...
            return (
              <SidebarMenuItem key={link.href}>
                <SidebarMenuButton
                  ...
                  tooltip={link.label}
                  className={cn(
                    "h-12 rounded-xl text-base font-medium text-sidebar-foreground/80 hover:bg-secondary/10 hover:text-secondary data-[active=true]:bg-secondary data-[active=true]:text-white data-[active=true]:font-bold data-[active=true]:ring-1 data-[active=true]:ring-secondary/20"
                  )}
                >
                  <Link href={link.href} className="flex items-center gap-3">
                    <span className={cn(
                      "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg transition-colors",
                      (isActive || isChildActive) ? "bg-secondary text-white" : "bg-sidebar-accent/60 text-muted-foreground group-hover/menu-button:text-secondary"
                    )}>
                      ...
                      <span className="flex min-w-0 flex-col gap-0.5 truncate">
                        ...
                        {link.description && (
                          <span className={cn(
                            ...
                            (isActive || isChildActive) && "font-medium text-white/80"
                          )}>
                            {link.description}
                          </span>
                        )}
                ...
                {hasChildren && sidebarState === 'expanded' && isExpanded && (
                  <SidebarMenuSub className="ml-[calc(theme(spacing.3)+theme(spacing.5)+theme(spacing.3))] border-l-2 border-secondary/30 pl-3">
                      ...
                      return (
                        <SidebarMenuSubItem key={child.href}>
                          <SidebarMenuSubButton
                            ...
                            className={cn(
                              "py-2 rounded-lg data-[active=true]:bg-secondary data-[active=true]:text-white data-[active=true]:font-bold"
                            )}
                          >
                            <Link href={child.href}>
                              <span className="flex flex-col gap-0.5 py-0.5">
                                ...
                                {child.secondary && (
                                  <span className={cn("truncate text-xs font-normal text-muted-foreground/70", childActive && "font-medium text-white/80")}>
                                    {child.secondary}
                                  </span>
                                )}
                              </span>
...
function LocalSidebarNavInner() {
  ...
  return (
    <Sidebar className="border-r border-sidebar-border" collapsible="icon">
      <SidebarHeader className="h-16 bg-gradient-to-r from-secondary/15 via-secondary/5 to-transparent border-b border-sidebar-border">
        <div className={cn("flex items-center h-full", sidebarState === 'expanded' ? "justify-between pl-3 pr-2" : "justify-center")}>
          {sidebarState === 'expanded' ? (
            <Link href="/local" className="flex items-center gap-3 overflow-hidden">
              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-secondary text-white ring-1 ring-secondary/20">
             ...
             <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-10 w-10 text-secondary hover:text-secondary/80 hover:bg-sidebar-accent"
